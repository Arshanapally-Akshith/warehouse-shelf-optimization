...
st.markdown("""
### 📌 Problem Statement
Optimize warehouse shelf placement to reduce travel distance and group frequently co-picked items.

### 🎯 Objective
Minimize total picking cost using:
...
st.markdown("""
### 📊 Insight
- Greedy improves quickly but gets stuck in local minima
- Simulated Annealing continues exploring and finds better solutions

👉 This demonstrates the difference between local and global optimization
""")
...
if st.button("Run Optimization"):
    ...
    col1.metric("SA Final Cost", round(sa_cost, 2))
    col1.metric("SA Improvement %", round((initial_cost - sa_cost)/initial_cost * 100, 2))
    col1.metric("SA Cost Reduction", round(initial_cost - sa_cost, 2))

    col2.metric("Greedy Final Cost", round(gr_cost, 2))
    col2.metric("Greedy Improvement %", round((initial_cost - gr_cost)/initial_cost * 100, 2))
    col2.metric("Greedy Cost Reduction", round(initial_cost - gr_cost, 2))
